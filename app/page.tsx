...
export default function KanaCrosswordApp(): React.ReactElement {
  ...
  function toHiragana(str:string): string {
    return Array.from(str).map((c: string)=>{
      ...
      if (code>=0xFF66 && code<=0xFF9D) { const fw = c.normalize('NFKC'); return toHiragana(fw); }
      return c;
    }).join('');
  }
  ...
  function parseInput(text:string): {items: Item[]; warnings: string[]} {
    ...
    const items: Item[] = []; const warnings: string[] = [];
    lines.forEach((line: string, i: number)=>{
      let kanji='', yomi='';
      const m = line.match(/^(.+?)[(（]([ぁ-んァ-ンー]+)[)）]$/);
      if (m){ kanji=m[1].trim(); yomi=m[2].trim(); }
      else if (line.includes('=')){
        const [a,b] = line.split('='); kanji=a.trim(); yomi=(b||'').trim();
      } else {
        const parts=line.split(/\s+/); if (parts.length>=2){ kanji=parts.slice(0,-1).join(' '); yomi=parts.at(-1)!; } else { kanji=line; yomi=''; }
      }
  ...
  function buildCrossword(items: Item[]): {grid: Map<string, Cell>; placements: Placement[]} {
    ...
    for (let wi=1; wi<items.length; wi++){
      ...
      if (best){
        ...
        // 交差不可なら独立配置（段をずらす）
        let maxY = 0; grid.forEach((_, key)=>{ const [,y]=fromKey(key); if (y>maxY) maxY=y; });
        ...
        else { placeWord(entry,'across', wi*3, sy); }
      }
    }
  ...
  function computeBounds(grid: Map<string, Cell>): Bounds {
    ...
    grid.forEach((_, key)=>{ const [x,y]=fromKey(key); if (x<minX) minX=x; if (x>maxX) maxX=x; if (y<minY) minY=y; if (y>maxY) maxY=y; });
  ...
  function computeClueNumbers(placements: Placement[], grid: Map<string, Cell>): Record<string, number> {
    const numbers: Record<string, number> = {}; let n=1;
    const filled = new Set<string>([...grid.keys()]);
    const has = (x:number,y:number)=> filled.has(keyOf(x,y));

    // よこ
    placements.filter(p=>p.dir==='across')
      .sort((a,b)=> a.startY===b.startY? a.startX-b.startX : a.startY-b.startY)
      .forEach(p=>{ if (!has(p.startX-1, p.startY)) numbers[p.id]=n++; });

    // たて（すでに振られていれば再利用）
    placements.filter(p=>p.dir==='down')
      .sort((a,b)=> a.startX===b.startX? a.startY-b.startY : a.startX-b.startX)
      .forEach(p=>{ if (!has(p.startX, p.startY-1)) numbers[p.id] = numbers[p.id] ?? n++; });
  ...
  function labelFromNumber(num:number): string {
    if (labelStyle==='alpha'){
      let n=num, s=''; while(n>0){ n--; s=String.fromCharCode(65+(n%26))+s; n=Math.floor(n/26);} return s;
    }
    return String(num);
  }
  ...
  React.useEffect(()=>{ rebuild(); }, []);

  // ==== Special cells ====
  const selectedLetters = React.useMemo<string[]>(()=>{
    const arr:string[]=[]; for (const k of selectedCells){ const c=grid.get(k); if (c) arr.push(c.ch); }
    return arr;
  }, [selectedCells, grid]);

  function toggleCellSpecial(k:string): void {
    setSelectedCells(prev=>{ const next=new Set(prev); if(next.has(k)) next.delete(k); else next.add(k); return next; });
  }
  ...
  function runTests(): void {
    ...
    // 3) 交差: すな × あな が交差する
    {
      const words: Item[] = [{id:'w0',kanji:'砂',hira:'すな'},{id:'w1',kanji:'穴',hira:'あな'},{id:'w2',kanji:'視点',hira:'してん'}];
      const {grid:g} = buildCrossword(words);
      let overlap=false; g.forEach(c=>{ const ids=new Set(c.words.map(w=>w.id)); if(ids.has('w0')&&ids.has('w1')) overlap=true; });
    ...
    // 4) 独立配置: してん は他と交差しない（この例では）
    {
      const words: Item[] = [{id:'w0',kanji:'砂',hira:'すな'},{id:'w1',kanji:'穴',hira:'あな'},{id:'w2',kanji:'視点',hira:'してん'}];
      const {grid:g} = buildCrossword(words);
      let w2Overlap=false; g.forEach(c=>{ const ids=new Set(c.words.map(w=>w.id)); if(ids.has('w2')&&(ids.has('w0')||ids.has('w1'))) w2Overlap=true; });
      results.push({name:'独立配置: してん', pass: !w2Overlap});
    }
    // 5) ラベル A,B,...,Z,AA の生成
    {
      const conv=(n:number)=>{ let s=''; let nn=n; while(nn>0){ nn--; s=String.fromCharCode(65+(nn%26))+s; nn=Math.floor(nn/26);} return s; };
  ...
  placements.forEach((p)=>{ const k=keyOf(p.startX,p.startY); const num=clueNums[p.id]; if(num) startNumByCell.set(k, num); });

  function GridView({showLetters, allowSelect}:{showLetters:boolean; allowSelect:boolean}): JSX.Element {
    return (
      <div className='relative inline-block'>
        <div className='bg-white rounded-md shadow border overflow-hidden' style={{ width: width*cellSize, height: height*cellSize }}>
          {Array.from({length:height}).map((_,ry)=> (
            <div key={ry} className='flex'>
              {Array.from({length:width}).map((_,rx)=>{
                const x=bounds.minX+rx, y=bounds.minY+ry; const k=keyOf(x,y); const cell=grid.get(k); const has=!!cell;
                const isSpecial = selectedCells.has(k);
                return (
                  <div
                    key={rx}
                    onClick={()=> allowSelect && has && toggleCellSpecial(k)}
                    className={`relative flex items-center justify-center select-none ${allowSelect && has ? 'cursor-pointer':''}`}
                    style={{width:cellSize,height:cellSize,borderRight:'1px solid #cbd5e1',borderBottom:'1px solid #cbd5e1',background:has?'#fff':'#e5e7eb'}}
                  >
                    {startNumByCell.has(k) && (
                      <div className='absolute top-0.5 left-1 text-[10px] text-gray-500'>{labelFromNumber(startNumByCell.get(k)!)}</div>
                    )}
                    {has && (
                      <div className={`text-lg ${isSpecial? 'font-extrabold':'font-medium'} ${showLetters? 'text-gray-900':'text-transparent'}`}>{cell!.ch}</div>
                    )}
                    {has && isSpecial && (<div className='pointer-events-none absolute inset-0 border-4 border-rose-500 rounded-[6px]' />)}
                  </div>
                );
              })}
            </div>
          ))}
          <div className='pointer-events-none absolute inset-0' style={{borderTop:'1px solid #cbd5e1', borderLeft:'1px solid #cbd5e1'}}/>
        </div>
      </div>
    );
  }

  function ClueList(): JSX.Element {
    const across = placements.filter(p=>p.dir==='across').sort((a,b)=> (clueNums[a.id]||0)-(clueNums[b.id]||0));
    const down   = placements.filter(p=>p.dir==='down').sort((a,b)=> (clueNums[a.id]||0)-(clueNums[b.id]||0));
    return (
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-3'>
        <div className='bg-white rounded-md border p-3'>
          <div className='font-bold mb-1'>よこ（Across）</div>
          <ul className='space-y-1'>
            {across.map(p=> (<li key={p.id}><span className='font-semibold mr-1'>{labelFromNumber(clueNums[p.id])}.</span>{p.kanji}</li>))}
        ...
        <div className='bg-white rounded-md border p-3'>
          ...
          <ul className='space-y-1'>
            {down.map(p=> (<li key={p.id}><span className='font-semibold mr-1'>{labelFromNumber(clueNums[p.id])}.</span>{p.kanji}</li>))}
  ...
  return (
    <div className='p-4'>
      <div className='flex gap-2 mb-4'>
        ...
        {/* 入力タブのときだけ右側に クリア/例 を出す */}
        {tab==='input' && (
          <div className='flex items-center gap-2 ml-auto'>
            <button onClick={()=>{ setRawInput(''); setPuzzlePrompt(''); setPuzzleAnswer(''); }} className='px-3 py-1.5 border rounded'>クリア</button>
            <button onClick={()=>{ setRawInput(exampleText); setPuzzlePrompt(examplePrompt); setPuzzleAnswer(exampleAnswer); }} className='px-3 py-1.5 border rounded'>例</button>
          </div>
        )}
        {tab!=='input' && (
          ...
          <div className='mb-3'>
            ...
            <div className='flex gap-2'>
              <input
                ...
                placeholder='例：よみクロスワード'
              />
              <button
                type='button'
                className='px-3 py-1.5 border rounded'
                onClick={()=> setTitle('よみクロスワード')}
                title='既定値に戻す'
              >
                既定に戻す
              </button>
